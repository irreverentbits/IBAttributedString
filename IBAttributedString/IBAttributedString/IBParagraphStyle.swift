...
//  IBAttributedString
//
//  Created by Lee Calloway on 10/17/17.
...
public enum IBParagraphStyle {
	...
	*/
	static func paragraphStyle(for styles: [IBParagraphStyle], overwriting existingStyle: NSParagraphStyle? = nil) -> NSParagraphStyle {
		...
		for style in styles {
			switch style {
			case .lineSpacing(let spacing):				newStyle.lineSpacing = spacing
			case .spacing(let spacing):					newStyle.paragraphSpacing = spacing
			case .alignment(let alignment):				newStyle.alignment = alignment
			case .firstLineHeadIndent(let indent):		newStyle.firstLineHeadIndent = indent
			case .headIndent(let indent):				newStyle.headIndent = indent
			case .tailIndent(let indent):				newStyle.tailIndent = indent
			case .lineBreakMode(let mode):				newStyle.lineBreakMode = mode
			case .minimumLineHeight(let height):		newStyle.minimumLineHeight = height
			case .maximumLineHeight(let height):		newStyle.maximumLineHeight = height
			case .baseWritingDirection(let direction):	newStyle.baseWritingDirection = direction
			case .lineHeightMultiple(let multiple):		newStyle.lineHeightMultiple = multiple
			case .spacingBefore(let spacing):			newStyle.paragraphSpacingBefore = spacing
			case .hyphenationFactor(let factor):		newStyle.hyphenationFactor = factor
			case .tabStops(let tabs):					newStyle.tabStops = tabs
			case .defaultTabInterval(let interval):		newStyle.defaultTabInterval = interval
			case .allowsDefaultTighteningForTruncation(let allowed):	newStyle.allowsDefaultTighteningForTruncation = allowed
			}
		}
